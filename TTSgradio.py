...
def add_audio_and_subtitles_to_video(input_video_path, subtitle_file_path, audio_file_path, output_video_path):
    # # Construct the ffmpeg command with GPU acceleration
    ...
    # last_sub_end_time = subs[-1].end.to_time() if subs else 0

    # # Apply subtitles
    # video_with_subtitles = ffmpeg.filter_(input_video, 'subtitles', subtitle_file_path)
    # # Trim the video to end at the last subtitle
    # video_with_subtitles = ffmpeg.trim(video_with_subtitles, end=last_sub_end_time)
    # # Combine video with new audio
    # output = ffmpeg.output(video_with_subtitles, input_audio, output_video_path, 
    #                        vcodec='h264_nvenc', acodec='aac', 
    #                        bitrate='5M', strict='experimental')
        ...
        with gr.Tab("TOOL lay SUB"):
            ...
            with gr.Row():
                with gr.Column():
                    ...
                    language_select = gr.Dropdown(label="Select Language", choices=["ja","auto"], value="ja")
